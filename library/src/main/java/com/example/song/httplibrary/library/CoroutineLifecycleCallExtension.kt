...
/**
 * @package com.example.song.httplibrary.library
 * @fileName LifecycleCallExtension
...
internal const val CoroutineLifecycleCallExtensionKtPath =
    "com.example.song.httplibrary.library.CoroutineLifecycleCallExtensionKt"
            ...
            override fun onSuccess(statusCode: Int, response: Response<T>) {
                cancellableContinuation.resumeWith(runCatching {
                    if (response.isSuccessful) {
                        ...
                        HttpResult.Error(
                            NetworkError(response.code(), response.message(), HttpException(response))
                        )
                    }
                })
            }
